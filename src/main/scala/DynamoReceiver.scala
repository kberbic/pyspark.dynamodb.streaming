 ...
 *
 * @param streamArn   DynamoDB stream name
 ...
 */
private[kinesis] class DynamoReceiver[T](
   ...
   */
  @volatile private var kinesisCheckpointer: DynamoCheckpointer = null
  ...
  override def onStart() {
    ...
    kinesisCheckpointer = new DynamoCheckpointer(receiver, checkpointInterval, workerId)
    val kinesisProvider = kinesisCreds.provider
      ...
      val kinesisClientLibConfiguration = new KinesisClientLibConfiguration(
        ...
        kinesisProvider,
        workerId)
        .withTaskBackoffTimeMillis(taskBackoffTimeMillis)
    ...
    */
    val recordProcessorFactory = new IRecordProcessorFactory {
      override def createProcessor: IRecordProcessor =
        new DynamoRecordProcessor(receiver, workerId)
    }
    val adapterClient:AmazonDynamoDBStreamsAdapterClient = new AmazonDynamoDBStreamsAdapterClient(kinesisClientLibConfiguration.getKinesisCredentialsProvider(), new ClientConfiguration())
    val dynamoDBClient:AmazonDynamoDBClient = new AmazonDynamoDBClient(kinesisClientLibConfiguration.getDynamoDBCredentialsProvider(), new ClientConfiguration())
    val cloudWatchClient:AmazonCloudWatchClient = new AmazonCloudWatchClient(kinesisClientLibConfiguration.getCloudWatchCredentialsProvider(), new ClientConfiguration())

    adapterClient.setRegion(Region.getRegion(Regions.fromName(regionName)))
    dynamoDBClient.setRegion(Region.getRegion(Regions.fromName(regionName)))
  ...
  private[kinesis] def addRecords(shardId: String, records: java.util.List[Record]): Unit = {
    if (records.size > 0) {
      ...
      val metadata = SequenceNumberRange(streamArn, shardId,
