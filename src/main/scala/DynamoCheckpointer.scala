 ...
 *
 * @param 
import org.apache.spark.streaming.kinesis.DynamoDBStream.DynamoRecordProcessorreceiver The receiver that keeps track of which sequence numbers we can checkpoint
 ...
 */
private[kinesis] class DynamoCheckpointer(
    receiver: DynamoReceiver[_],
  ...
  def removeCheckpointer(shardId: String, checkpointer: IRecordProcessorCheckpointer): Unit = {
    ...
    if (checkpointer != null) {
      try {
        ...
        DynamoRecordProcessor.retryRandom(checkpointer.checkpoint(), 4, 100)
      } catch {
        case NonFatal(e) =>
      ...
      if (checkpointer != null) {
        receiver.getLatestSeqNumToCheckpoint(shardId).foreach { latestSeqNum =>
          ...
          if (lastSeqNum == null || latestSeqNum > lastSeqNum) {
            /* Perform the checkpoint */
            DynamoRecordProcessor.retryRandom(checkpointer.checkpoint(latestSeqNum), 4, 100)
