 ...
 */
private[kinesis] class DynamoRecordProcessor[T](receiver: DynamoReceiver[T], workerId: String)
  ...
  override def initialize(initializationInput: InitializationInput) {
    ...
    logInfo(s"Initialized workerId $workerId with shardId $this.shardId")
  }
  ...
  override def processRecords(processRecordsInput: ProcessRecordsInput) {
    if (!receiver.isStopped()) {
      try {
        ...
        for (start <- 0 until processRecordsInput.getRecords().size() by maxRecords) {
          val miniBatch = processRecordsInput.getRecords().subList(start, math.min(start + maxRecords, processRecordsInput.getRecords().size()))
        ...
        receiver.setCheckpointer(shardId, processRecordsInput.getCheckpointer())
      } catch {
        case NonFatal(e) =>
          /*
  ...
  override def shutdown(shutdownInput: ShutdownInput): Unit = {
    logInfo(s"Shutdown:  Shutting down workerId $workerId with reason $shutdownInput.getShutdownReason()")
    ...
    if (shardId == null) {
      ...
      shutdownInput.getShutdownReason() match {
         ...
         */
        case ShutdownReason.TERMINATE => receiver.removeCheckpointer(shardId, shutdownInput.getCheckpointer())
...
private[kinesis] object DynamoRecordProcessor extends Logging {
  /**
