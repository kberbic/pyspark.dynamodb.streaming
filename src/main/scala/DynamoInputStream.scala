...
class DynamoInputDStream[T: ClassTag](
  ...
  override def getReceiver(): Receiver[T] = {
    new DynamoReceiver(streamArn, endpointUrl, regionName, initialPosition,
      checkpointAppName, checkpointInterval, _storageLevel, maxRecords, taskBackoffTimeMillis, messageHandler,
      kinesisCreds, dynamoDBCreds, cloudWatchCreds)
  }
}

@InterfaceStability.Evolving
object DynamoInputDStream {
  /**
   * Builder for [[DynamoInputDStream]] instances.
   *
   * @since 2.2.0
   */
  @InterfaceStability.Evolving
  class Builder {
    ...
    private var streamArn: Option[String] = None
    ...
    private var taskBackoffTimeMillis: Option[Int] = None
    private var maxRecords: Option[Int] = None
     ...
     * @return Reference to this [[DynamoInputDStream.Builder]]
     */
     ...
     * parameter.
     *
     * @param streamArn Name of Kinesis stream that the DStream will read from
     * @return Reference to this [[DynamoInputDStream.Builder]]
     */
    def streamArn(streamArn: String): Builder = {
      this.streamArn = Option(streamArn)
      this
    }
    
    def maxRecords(maxRecords: Int): Builder = {
      this.maxRecords = Option(maxRecords)
      this
    }
    
    def taskBackoffTimeMillis(taskBackoffTimeMillis: Int): Builder = {
      this.taskBackoffTimeMillis = Option(taskBackoffTimeMillis)
      this
    }
     ...
     * @return Reference to this [[DynamoInputDStream.Builder]]
     */
    def initialPosition(initialPosition: InitialPositionInStream): Builder = {
     ...
     * @return Reference to this [[DynamoInputDStream.Builder]]
     */
    @deprecated("use initialPosition(initialPosition: InitialPositionInStream)", "2.3.0")
    ...
    /**
     * Create a new instance of [[DynamoInputDStream]] with configured parameters and the provided
     * message handler.
     *
     * @param handler Function converting [[Record]] instances read by the KCL to DStream type [[T]]
     * @return Instance of [[DynamoInputDStream]] constructed with configured parameters
     */
    def buildWithMessageHandler[T: ClassTag](
        handler: Record => T): DynamoInputDStream[T] = {
      val ssc = getRequiredParam(streamingContext, "streamingContext")
      new DynamoInputDStream[T](
        ssc,
        getRequiredParam(streamArn, "streamArn"),
        endpointUrl.getOrElse(DEFAULT_KINESIS_ENDPOINT_URL),
        regionName.getOrElse(DEFAULT_KINESIS_REGION_NAME),
        initialPosition.getOrElse(InitialPositionInStream.LATEST),
        ...
        maxRecords.getOrElse(DEFAULT_KINESIS_MAX_RECORDS),
        taskBackoffTimeMillis.getOrElse(DEFAULT_KINESIS_TASK_BACKOFF_TIME_MILLIS),
    ...
    /**
     * Create a new instance of [[DynamoInputDStream]] with configured parameters and using the
     * default message handler, which returns [[Array[Byte]]].
     *
     * @return Instance of [[DynamoInputDStream]] constructed with configured parameters
     */
    def build(): DynamoInputDStream[Array[Byte]] = buildWithMessageHandler(defaultMessageHandler)
  ...
  /**
   * Creates a [[DynamoInputDStream.Builder]] for constructing [[DynamoInputDStream]] instances.
   *
   * @since 2.2.0
   * @return [[DynamoInputDStream.Builder]] instance
   */
  ...
  private[kinesis] val DEFAULT_KINESIS_MAX_RECORDS: Int = 10
  private[kinesis] val DEFAULT_KINESIS_TASK_BACKOFF_TIME_MILLIS: Int = 500
